...
def parse_natural_language(text, available_columns, file_schemas):
    ...
    # -----------------------------------------
    # PHASE 5.1A: Explicit chart type detection
    # -----------------------------------------
    chart_keywords = {
        "line chart": "line",
        "line": "line",
    ...
    for phrase, chart_type in chart_keywords.items():
        if phrase in text:
            intent["chart_type"] = chart_type
            print(">>> CHART TYPE DETECTED (explicit):", chart_type)
        break
    # ---------------------------
    ...
    # ---------------------------------------------------------
    # PHASE 5.3: Temporal filter extraction
    # ---------------------------------------------------------


    def add_temporal_filter(column, operator, value):
    ...
    if match:
        ...
        add_temporal_filter("__TEMPORAL__", "between", [start.isoformat(), end.isoformat()])
        print(">>> TEMPORAL FILTER: last", days, "days")


    # 2. "past X hours"
    match = re.search(r"past (\d+) hours", text)
    if match:
        hours = int(match.group(1))
        end = datetime.today()
        start = end - timedelta(hours=hours)
        add_temporal_filter("__TEMPORAL__", "between", [start.isoformat(), end.isoformat()])
        print(">>> TEMPORAL FILTER: past", hours, "hours")
    ...
    if "last month" in text:
        ...
        add_temporal_filter("__TEMPORAL__", "between", [first_day.isoformat(), last_day.isoformat()])
        print(">>> TEMPORAL FILTER: last month")
    ...
    if "this year" in text:
        ...
        add_temporal_filter("__TEMPORAL__", "between", [start.isoformat(), end.isoformat()])
        print(">>> TEMPORAL FILTER: this year")


    # 5. "between day X and day Y"
    match = re.search(r"between day (\d+) and day (\d+)", text)
    if match:
        start = int(match.group(1))
        end = int(match.group(2))
        add_temporal_filter("__TEMPORAL__", "between", [start, end])
        print(">>> TEMPORAL FILTER: between day", start, "and", end)


    # 6. "from <date> to <date>"
    match = re.search(r"from ([\w\-\/]+) to ([\w\-\/]+)", text)
    if match:
        start = match.group(1)
        end = match.group(2)
        add_temporal_filter("__TEMPORAL__", "between", [start, end])
        print(">>> TEMPORAL FILTER: from", start, "to", end)
