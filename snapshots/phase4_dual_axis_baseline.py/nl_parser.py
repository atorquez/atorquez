...
def parse_natural_language(text, available_columns):
    ...
    if explicit_metrics:
        ...
        final_metrics = []  
    # ---------------------------------------------------
    # PHASE 4: Dual-axis detection (AFTER final_metrics exists)
    # ---------------------------------------------------
    ...
    if len(final_metrics) >= 2 and user_requests_dual:
        ...
        print(">>> DUAL AXIS DETECTED:", intent["y_axis_left"], intent["y_axis_right"])
    # DO NOT RETURN — allow chart_type detection to run
    else:
    # Phase 3 fallback
